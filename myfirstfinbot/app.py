...
if submit_button:
    with st.spinner("Analyzing stock data..."):
        try:
            ...
            # Create recent dataframe for visualization
            recent_df = ohlcv.copy().tail(100)
            recent_df = recent_df.reset_index()
            
            # Prepare derived data for trading decisions
            latest_price = recent_df["Close"].iloc[-1]
            
            # --- SIMPLIFIED PATTERN SUMMARY ---
            # Show a concise summary of all detected patterns
            with st.expander("Pattern Detection Summary", expanded=True):
